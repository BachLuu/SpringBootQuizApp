 ...
 * Provides complex statistics and reporting capabilities.
 * Uses Interface-Based Projections for type-safe query results.
 */
@Service
...
public class AnalyticsService implements IAnalyticsService {
    ...
    private List<QuestionPerformanceDto> getQuestionPerformanceList(UUID quizId) {
        ...
        return results.stream()
                .map(proj -> {
                    ...
                    String difficultyLevel = calculateDifficultyLevel(correctRate);

                    return new QuestionPerformanceDto(
                            proj.getQuestionId(),
                            truncateString(proj.getContent(), 100),
                            proj.getType() != null ? proj.getType().name() : "UNKNOWN",
                            total,
                            correct,
                            nullSafe(proj.getIncorrectAnswers()),
                            correctRate,
                            toInteger(proj.getAvgTime()),
                            difficultyLevel
                    );
                })
    ...
    private List<QuestionTypePerformanceDto> buildQuestionTypePerformance(UUID userId) {
        ...
        return results.stream()
                .map(proj -> {
                    ...
                    return new QuestionTypePerformanceDto(
                            proj.getType() != null ? proj.getType().name() : "UNKNOWN",
                            total,
                            correct,
                            accuracy,
    ...
    private List<RecentQuizAttemptDto> buildRecentAttempts(UUID userId) {
        ...
        return sessions.stream()
                .map(session -> {
                    String quizTitle = quizRepository.findById(session.getQuizId())
                            .map(Quiz::getTitle)
                            .orElse("Unknown Quiz");

                    return new RecentQuizAttemptDto(
                            session.getId(),
                            session.getQuizId(),
                            quizTitle,
                            session.getScore(),
    ...
    private List<ProgressDataDto> buildProgressOverTime(UUID userId) {
        ...
        return results.stream()
                .map(proj -> new ProgressDataDto(
                        proj.getPeriod(),
                        nullSafe(proj.getQuizzesTaken()),
                        nullSafe(proj.getAvgScore()),
                        toBigDecimal(proj.getPassRate())
                ))
                .collect(Collectors.toList());
    }
    ...
    private SystemOverviewDto buildSystemOverview() {
        ...
        return new SystemOverviewDto(
                ...
                totalAttempts,
                completedAttempts,
                overallPassRate.setScale(2, RoundingMode.HALF_UP)
        );
    }
    ...
    private ActivityStatsDto buildActivityStats() {
        ...
        return new ActivityStatsDto(
                nullSafe(activityStats.getAttemptsToday()),
                nullSafe(activityStats.getAttemptsThisWeek()),
                nullSafe(activityStats.getAttemptsThisMonth()),
                nullSafe(newUsersThisWeek),
                nullSafe(newUsersThisMonth),
    ...
    private List<PopularQuizDto> buildPopularQuizzes() {
        ...
        return results.stream()
                .map(proj -> {
                    String quizTitle = quizRepository.findById(proj.getQuizId())
                            .map(Quiz::getTitle)
                            .orElse("Unknown Quiz");

                    return new PopularQuizDto(
                            proj.getQuizId(),
                            quizTitle,
                            nullSafe(proj.getTotalAttempts()),
        ...
        return results.stream()
                .map(proj -> {
                    ...
                    return new QuizCompletionDto(
                            ...
                            totalAttempts,
                            completedAttempts,
                            nullSafe(proj.getAbandonedAttempts()),
                            completionRate,
                            BigDecimal.ZERO // pass rate - would need separate query
                    );
                })
                .collect(Collectors.toList());
    }
    ...
    public QuestionDifficultyResponse getQuestionDifficultyAnalysis(UUID questionId) {
        ...
        return new QuestionDifficultyResponse(
                questionId,
                question.getContent(),
    ...
    private DifficultyMetricsDto buildDifficultyMetrics(UUID questionId) {
        ...
        return new DifficultyMetricsDto(
                totalAttempts,
                correctAttempts,
                incorrectAttempts,
                skippedAttempts,
    ...
    private String calculateDifficultyLevel(BigDecimal correctRate) {
        ...
        if (correctRate.compareTo(VERY_EASY_THRESHOLD) >= 0) {
            return "VERY_EASY";
        } else if (correctRate.compareTo(EASY_THRESHOLD) >= 0) {
            return "EASY";
        } else if (correctRate.compareTo(MEDIUM_THRESHOLD) >= 0) {
            return "MEDIUM";
        } else if (correctRate.compareTo(HARD_THRESHOLD) >= 0) {
            return "HARD";
        } else {
            return "VERY_HARD";
        }
