...
public class QuizSessionService implements IQuizSessionService {
    ...
    public void abandonSession(UUID sessionId, UUID userId) {
        ...
        if (session.getStatus() == QuizSessionStatus.SUBMITTED ||
            session.getStatus() == QuizSessionStatus.GRADED) {
    ...
    public QuizQuestionResponse getQuestionByIndex(UUID sessionId, int questionIndex, UUID userId) {
        ...
        List<QuizQuestionResponse.QuizAnswerOption> options = question.getAnswers().stream()
    ...
    public SubmitAnswerResponse submitAnswer(UUID sessionId, SubmitAnswerRequest request, UUID userId) {
        ...
        Question question = quiz.getQuizQuestions().stream()
    ...
    public QuizSessionResultResponse submitQuiz(UUID sessionId, UUID userId) {
        ...
        if (session.getStatus() != QuizSessionStatus.IN_PROGRESS &&
            session.getStatus() != QuizSessionStatus.PAUSED) {
    ...
    public QuizSessionResultResponse getResult(UUID sessionId, UUID userId) {
        ...
        List<SessionAnswer> sessionAnswers = sessionAnswerRepository
                .findByQuizSessionIdOrderByAnswerOrder(sessionId);
        ...
        for (QuizQuestion qq : quiz.getQuizQuestions()) {
            ...
            Answer selectedAnswer = sa != null && sa.getAnswerId() != null ?
                    question.getAnswers().stream()
                            .filter(a -> a.getId().equals(sa.getAnswerId()))
                            .findFirst().orElse(null) : null;

            // All options
            List<QuizSessionResultResponse.AnswerOption> allOptions = question.getAnswers().stream()
                    .filter(Answer::getIsActive)
                    .map(a -> new QuizSessionResultResponse.AnswerOption(
                            a.getId(),
                            a.getContent(),
                            a.getIsCorrect(),
            ...
            answerResults.add(new QuizSessionResultResponse.AnswerResultResponse(
                    question.getId(),
                    question.getContent(),
                    question.getQuestionType().name(),
                    sa != null ? sa.getAnswerId() : null,
                    ...
                    sa != null ? sa.getTextResponse() : null,
                    sa != null && Boolean.TRUE.equals(sa.getIsCorrect()),
                    sa != null ? sa.getPointsAwarded() : BigDecimal.ZERO,
                    POINTS_PER_QUESTION,
                    sa != null ? sa.getTimeSpentSeconds() : 0,
                    sa != null && Boolean.TRUE.equals(sa.getIsReviewed()),
                    sa != null ? sa.getReviewerFeedback() : null,
                    allOptions
            ));
        }
        ...
        QuizSessionResultResponse.QuizStatistics statistics = new QuizSessionResultResponse.QuizStatistics(
                ...
                avgTimePerQuestion,
                null, // Could calculate fastest/slowest
                null,
                rank,
                (int) totalParticipants
        );

        return new QuizSessionResultResponse(
                session.getId(),
                quiz.getId(),
                quiz.getTitle(),
                quiz.getDescription(),
                ...
                session.getTotalQuestions() - session.getAnsweredQuestions(),
                session.getScore(),
                session.getPointsEarned(),
                session.getMaxPoints(),
                session.getIsPassed(),
                PASSING_SCORE,
                answerResults,
                statistics
    ...
    public Page<QuizSessionSummaryResponse> getUserHistory(UUID userId, int page, int size) {
        ...
        return quizSessionRepository.findByUserIdOrderByCreatedAtDesc(userId, pageable)
                .map(this::mapToSummary);
    }
    ...
    private int calculateTimeSpent(QuizSession session) {
        ...
        LocalDateTime endTime = session.getFinishedAt() != null ?
    ...
    private void calculateFinalScore(QuizSession session) {
        ...
        for (SessionAnswer answer : answers) {
            if (Boolean.TRUE.equals(answer.getIsCorrect())) {
                correct++;
                pointsEarned = pointsEarned.add(answer.getPointsAwarded() != null ?
                        answer.getPointsAwarded() : BigDecimal.ZERO);
            }
        }
    ...
    private QuizSessionSummaryResponse mapToSummary(QuizSession session) {
        ...
        return new QuizSessionSummaryResponse(
                ...
                quiz != null ? quiz.getThumbnailUrl() : null,
                session.getStatus(),
                session.getStartedAt(),
                session.getFinishedAt(),
                session.getTimeSpentSeconds(),
    ...
    private LeaderboardResponse buildLeaderboard(Quiz quiz, List<QuizSession> sessions) {
        ...
        List<LeaderboardResponse.LeaderboardEntry> entries = sessions.stream()
                .map(session -> {
                    User user = userRepository.findById(session.getUserId()).orElse(null);
